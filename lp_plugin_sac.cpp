...
const std::string kLogFile = "/home/cpii/projects/log/gym_1d_vision/test8/tfevents.pb";

const QString memoryPath("/home/cpii/storage_d1/RL1/SAC/gym_1d_vision/memory");
const QString modelPath("/home/cpii/storage_d1/RL1/SAC/gym_1d_vision/models");
...
bool LP_Plugin_Sac::eventFilter(QObject *watched, QEvent *event)
{
...
void LP_Plugin_Sac::savedata(QString fileName, std::vector<float> datas){
...
void LP_Plugin_Sac::loaddata(std::string fileName, std::vector<float> &datas){
    // Open the File
...
void LP_Plugin_Sac::drawcart(torch::Tensor &state, int thickness, int step){
    std::vector<float> state_vector = std::vector(state.data_ptr<float>(), state.data_ptr<float>() + state.numel());
    float cart_posi = state_vector[0];
    float rod_theta = state_vector[2];

    cv::Mat image(128, 128, CV_8UC3, cv::Scalar(0, 0, 0));

    // Top Left Corner
    cv::Point p1(32+32/x_threshold*cart_posi, 96);

    // Bottom Right Corner
    cv::Point p2(96+32/x_threshold*cart_posi, 112);
    ...
    cv::Point rod[1][4];
    float theta = 0.5*(rod_theta+1)*M_PI;
    float x_1 = 64+32/x_threshold*cart_posi-8*sin(theta);
    float x_2 = 64+32/x_threshold*cart_posi+8*sin(theta);
    float y_1 = 104+8*cos(theta);
    float y_2 = 104-8*cos(theta);
    float rodx = 64+32/x_threshold*cart_posi-80*cos(theta);
    float rody = 104-80*sin(theta);
    rod[0][0] = cv::Point(x_1, y_1);
    rod[0][1] = cv::Point(x_2, y_2);
    rod[0][2] = cv::Point(rodx+8*sin(theta), rody-8*cos(theta));
    rod[0][3] = cv::Point(rodx-8*sin(theta), rody+8*cos(theta));

    const cv::Point* ppt[1] = { rod[0] };
    int npt[] = { 4 };
    ...
    gLock.lockForWrite();
    gImage = QImage((uchar*) image.data, image.cols, image.rows, image.step, QImage::Format_BGR888).copy();
    gLock.unlock();
    emit glUpdateRequest();

    state = torch::from_blob(image.data, { image.rows, image.cols, image.channels() }, at::kByte);
    state = state.permute({ 2, 0, 1 })/255.0;
    state = state.index({torch::indexing::Slice(1, 2)});
    torch::Tensor tmp = state.clone().detach();
//    std::cout << "state size: " << state.sizes() << std::endl
//              << "min: " << torch::min(state) << std::endl
//              << "max: " << torch::max(state) << std::endl;
    if(step==0){
        state = torch::cat({ state, state }, 0);
    } else {
        state = torch::cat({ tmp_state, state }, 0);
    }
    tmp_state = tmp.clone().detach();
}

void LP_Plugin_Sac::Reinforcement_Learning(){
        ...
        try {
            ...
            while(episode < maxepisode){
                ...
                while(step < maxstep){
                    ...
                    drawcart(after_state, thickness, step);
...
void LP_Plugin_Sac::FunctionalRender_L(QOpenGLContext *ctx, QSurface *surf, QOpenGLFramebufferObject *fbo, const LP_RendererCam &cam, const QVariant &options)
{
    Q_UNUSED(surf)
    Q_UNUSED(options)
    ...
    if(mTraining){
        ...
        static std::vector<QVector3D> quad =
                                      {QVector3D( 64.0f, 0.0f, 0.0f),
                                       QVector3D( 64.0f, 128.0f, 0.0f),
                                       QVector3D(-64.0f, 128.0f, 0.0f),
                                       QVector3D(-64.0f, 0.0f, 0.0f)};
        ...
        mProgram_L->setAttributeArray("a_pos", quad.data());
        mProgram_L->setAttributeArray("a_tex", tex.data());

        gLock.lockForRead();
        QOpenGLTexture texture(gImage.mirrored());
        gLock.unlock();
...
void LP_Plugin_Sac::FunctionalRender_R(QOpenGLContext *ctx, QSurface *surf, QOpenGLFramebufferObject *fbo, const LP_RendererCam &cam, const QVariant &options)
{
    Q_UNUSED(surf)
    //Q_UNUSED(options)
...
void LP_Plugin_Sac::initializeGL_L()
{
        std::string vsh, fsh;
...
void LP_Plugin_Sac::initializeGL_R()
{
    std::string vsh, fsh;
...
QString LP_Plugin_Sac::MenuName()
{
    return tr("menuPlugins");
}

QAction *LP_Plugin_Sac::Trigger()
{
    if ( !mAction ){
        mAction = new QAction("Sac");
    }
    return mAction;
}
